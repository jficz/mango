...
| `single_tagset` | `0` | All monitors share one global set of tags: each tag is displayed on at most one monitor at a time and windows follow the monitor showing their tag. Viewing a tag that another monitor displays transfers it; the other monitor visibly switches to another tag (cycles degrade to a swap). Toggling on a tag that every bit of it is displayed by other monitors is a no-op. Inspired by the dwm/dwl singletagset patch. Windows on the special workspace (tag `00`) and global windows keep their normal per-monitor behavior. Incompatible with `tag_gather` (disabled automatically). |
| `single_tagset_evict_history` | `0` | With `single_tagset`, an evicted monitor tries to switch back to the tag it displayed before its current one (cascading through monitors when needed) instead of jumping to the first unused tag. |
