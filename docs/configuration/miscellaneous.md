...
| `single_tagset` | `0` | All monitors share one global set of tags: each tag is displayed on at most one monitor at a time and windows follow the monitor showing their tag. Viewing a tag that another monitor displays transfers it; the other monitor visibly switches to another tag (cycles degrade to a swap). Toggling on a tag that every bit of it is displayed by other monitors is a no-op. Inspired by the dwm/dwl singletagset patch. Windows on the special workspace (tag `00`) and global windows keep their normal per-monitor behavior. Incompatible with `tag_gather` (disabled automatically). Commands that target a specific monitor (`viewcrossmon`, `tagcrossmon`, ...) work as expected: viewing a tag owned elsewhere takes it over on the targeted monitor. |
| `single_tagset_evict` | `0` | With `single_tagset`, what an evicted monitor displays after another monitor takes its tag: `0` = first unused tag, `1` = restore the tag it displayed before its current one (falls back to unused), `2` = swap: take the tag the initiating monitor displayed before the switch (falls back to `1`). |
