...
| `single_tagset` | `0` | All monitors share one global set of tags: each tag is displayed on at most one monitor at a time and windows follow the monitor showing their tag. Taking a tag from another monitor evicts it (`single_tagset_evict`). Special workspace and global windows are unaffected; incompatible with `tag_gather`. Inspired by the dwm/dwl singletagset patch. |
